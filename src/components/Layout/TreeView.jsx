...
const TreeNode = ({
  ...
  const spanClass = `flex items-center gap-4 w-fit py-2 pl-2 rounded-lg text-[#227099] hover:text-white hover:bg-[#227099] hover:cursor-pointer duration-500 ease-in-out transform hover:-translate-x-2`;
  const activeClass = `flex items-center gap-4 w-fit p-2 rounded-lg bg-[#E3EAEF] text-[#227099] cursor-pointer transform -translate-x-2`;
  ...
  if (hasChildren) {
    ...
    content = (
      <ul
        ...
        className={` w-full rounded flex gap-2 text-sm font-medium font-tajawal py-2 px-1 text-[#227099] cursor-pointer  duration-500 ease-in-out hover:-translate-x-2 ${
          chosenNode ? "-translate-x-2 bg-mainBlue text-white ml-1" : ""
        }`}
      >
        <img
          src={nestedIcon}
          width={14}
          alt="nestedIcon"
          className={`${chosenNode ? "hidden" : "block"}`}
        />
        <li className={`" ${chosenNode ? "px-2" : ""}`}>{node[NodeName]}</li>
      </ul>
    );
  }
